...
for i in range(30):
    ...
    # balence the labels
    positive = [i for i in range(len(label_)) if label_[i,-1]==1]
    negative = [i for i in range(len(label_)) if label_[i,-1]==0]
    ...
    net = RNN(input_size=10,num_layers=2).to(device) #

    # set model
    criterion = torch.nn.BCELoss()                        #loss fuction
    optimizer = torch.optim.SGD(net.parameters(),lr=LR,momentum=0.9)  #optimiser
    cosine_lr = WarmupCosineLR(optimizer, 0.00001, LR, warm_epoch, epochs, 0.2) #cosine annealing
            ...
            if batch%1==0:
                ...
                with torch.no_grad():
                    for data in val_dl:
                        inputs, labels = data[0].to(device), data[1].to(device)
                    dev_out = net(inputs)
                    dev_recall,dev_f1,dev_precision = evaluate(dev_out,labels[:,-1])
                    dev_loss = criterion(dev_out, labels[:,-1])
...
auc_data.to_csv('auc_baseline_LSTM.csv')
