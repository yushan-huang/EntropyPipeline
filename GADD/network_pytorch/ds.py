...
with open('/home/yushan/dcarte_origin/gait/gaint/network_pytorch/gait_baseline_dataset_timewindow_20.csv') as f:
...
with open('/home/yushan/dcarte_origin/gait/gaint/network_pytorch/gait_entropy_dataset_timewindow_20.csv') as f:
    head = f.readline().strip().split(',')
    head[0]='index'
    datas = [[eval(ii) for ii in i.strip().split(',')] for i in f.readlines()]
    datas = np.array(datas)
...
#unsquezze
inputs1 = inputs1.reshape([-1,1,3]).astype('float32')
