...
for i in range(30):
    ...
    net = MLP(input_size=2).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch, data in enumerate(train_dl):
            inputs, labels = data[0].to(device).reshape((-1,2)), data[1].to(device)
            optimizer.zero_grad()
            out = net(inputs)
...
evaluation_final.columns = ['F1_tr','F1_ts','recall_tr','recall_ts','acc_tr','acc_ts']
evaluation_final.to_csv('evaluation_baseline_MLP.csv')
